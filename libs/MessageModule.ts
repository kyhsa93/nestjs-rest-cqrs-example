...
import { Global, Inject, Logger, Module, OnModuleDestroy, SetMetadata } from "@nestjs/common";
import { ModulesContainer } from "@nestjs/core";
import { IEvent } from "@nestjs/cqrs";
import { DeleteMessageCommand, ReceiveMessageCommand, SendMessageCommand, SQSClient } from "@aws-sdk/client-sqs";
import { CreateTopicCommand, SNSClient, PublishCommand } from '@aws-sdk/client-sns';
...
export const MessageHandler = (name: string) => SetMetadata<Symbol, MessageHandlerMetadata>(SQS_CONSUMER_METHOD, { name });
  ...
  async handleMessage(): Promise<void> {
    ...
    const message = (
      (JSON.parse(response[0].Body) as { Message?: string }).Message
        ? JSON.parse((JSON.parse(response[0].Body) as { Message: string }).Message)
        : JSON.parse(response[0].Body)
    ) as Message;
    RequestStorage.setRequestId(message.requestId);

    const handler = (await this.discover.controllerMethodsWithMetaAtKey<MessageHandlerMetadata>(SQS_CONSUMER_METHOD))
      .find(handler => handler.meta.name === message.name);
    if (!handler) throw new Error(`Message handler is not found. Message: ${JSON.stringify(message)}`);
    ...
    const controller = Array.from(this.modulesContainer.values())
      ...
      .find(wrapper => wrapper.name == handler.discoveredMethod.parentClass.name);
    if (!controller) throw new Error(`Message handling controller is not found. Message: ${JSON.stringify(message)}`);

    try {
      await handler.discoveredMethod.handler.bind(controller.instance)(message.body);
      await this.sqsClient.send(new DeleteMessageCommand({
        QueueUrl: Config.AWS_SQS_QUEUE_URL,
        ReceiptHandle: response[0].ReceiptHandle,
      }));
    } catch(error) {
      return this.logger.error(`Message handling error. Message: ${JSON.stringify(message)}. Error: ${error}`);
    }
    this.logger.log(`Message handling completed. Message: ${JSON.stringify(message)}`);
  }
...
class SNSMessagePublisher {
  ...
  async publish(Name: Topic, Message: Message): Promise<void> {
    const message = {
      TopicArn: (await this.snsClient.send(new CreateTopicCommand({ Name }))).TopicArn,
...
class IntegrationEventPublisherImplement implements IntegrationEventPublisher {
  ...
  async publish(name: Topic, body: IEvent): Promise<void> {
    await this.snsMessagePublisher.publish(name, {
      name, body, requestId: RequestStorage.getStorage().requestId,
    })
  }
}
