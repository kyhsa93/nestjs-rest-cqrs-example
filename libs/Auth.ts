...
import { EntityIdTransformer, ENTITY_ID_TRANSFORMER, readConnection } from "libs/DatabaseModule";
...
class AuthGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const [accountId, password] = Buffer.from(base64, "base64").toString("utf8").split(":", 2);
...
export const Auth = () => applyDecorators(
  UseGuards(AuthGuard),
  ApiBasicAuth(),
  ApiForbiddenResponse({ description: "Authorization header validation is failed" }),
);
