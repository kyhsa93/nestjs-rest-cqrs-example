...
import {
  Account,
  AccountFindConditions,
  Accounts,
  AccountsAndCount,
  AccountWhereConditions,
  Query,
...
export default class AccountQuery implements Query {
  ...
  private convertAccountsAndCount([entities, count]: [
    AccountEntity[],
    number,
  ]): AccountsAndCount {
  ...
  private convertWhereConditions(
    conditions: AccountWhereConditions,
  ): undefined | FindConditions<AccountEntity> {
    const result = {};
    conditions.names.length === 0
      ? undefined
      : Object.assign(result, { name: In(conditions.names) });
    return Object.keys(result).length === 0 ? undefined : result;
  }

  private convertFindConditions({
    take,
    page,
    where,
  }: AccountFindConditions): FindManyOptions<AccountEntity> {
    return {
      skip: take * (page - 1),
      take,
      where: where ? this.convertWhereConditions(where) : undefined,
    };
  }

  public async findAndCount(
    conditions: AccountFindConditions,
  ): Promise<AccountsAndCount> {
    return getRepository(AccountEntity)
      .findAndCount(this.convertFindConditions(conditions))
      .then((entitiesAndCount) =>
        this.convertAccountsAndCount(entitiesAndCount),
      );
  }
}
