...
export class SendEmailHandler implements ICommandHandler<SendEmailCommand, void> {
  ...
  @Inject(InjectionToken.NOTIFICATION_REPOSITORY) private readonly notificationRepository: NotificationRepository;
  @Inject(InjectionToken.EMAIL_ADAPTOR) private readonly emailAdaptor: EmailAdaptor;

  @Transactional()
  async execute(command: SendEmailCommand): Promise<void> {
    const notification = this.notificationFactory.create({
      ...command, id: this.notificationRepository.newId(),
    });
    await this.notificationRepository.save(notification);
    await this.emailAdaptor.sendEmail(command.to, command.subject, command.content)
  }
}
