...
import { Notification, NotificationProperties } from "src/notification/domain/Notification";
...
export class NotificationRepositoryImplement implements NotificationRepository {
  ...
  async save(notification: Notification): Promise<void> {
    await writeConnection.manager.getRepository(NotificationEntity)
    .save(this.modelToEntity(notification));
  }

  private modelToEntity(model: Notification): NotificationEntity {
    const properties = JSON.parse(JSON.stringify(model)) as NotificationProperties;
