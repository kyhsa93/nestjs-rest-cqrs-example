...
import { AccountClosed, IntegrationEventPublisher, INTEGRATION_EVENT_PUBLISHER, Topic } from "libs/MessageModule";
...
export class AccountClosedHandler implements IEventHandler<AccountClosedEvent> {
  ...
  async handle(event: AccountClosedEvent): Promise<void> {
    await this.integrationEventPublisher.publish(Topic.ACCOUNT_CLOSED, new AccountClosed(event.accountId, event.email));
  }
}
