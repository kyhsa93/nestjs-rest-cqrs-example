...
import { AccountWithdrawn, IntegrationEventPublisher, INTEGRATION_EVENT_PUBLISHER, Topic } from "libs/MessageModule";
...
export class WithdrawnHandler implements IEventHandler<WithdrawnEvent> {
  ...
  async handle(event: WithdrawnEvent): Promise<void> {
    await this.integrationEventPublisher.publish(Topic.ACCOUNT_WITHDRAWN, new AccountWithdrawn(event.accountId, event.email));
  }
}
