...
import { AccountDeposited, IntegrationEventPublisher, INTEGRATION_EVENT_PUBLISHER, Topic } from "libs/MessageModule";
...
export class DepositedHandler implements IEventHandler<DepositedEvent> {
  ...
  async handle(event: DepositedEvent): Promise<void> {
    await this.integrationEventPublisher.publish(Topic.ACCOUNT_DEPOSITED, new AccountDeposited(event.accountId, event.email));
  }
}
