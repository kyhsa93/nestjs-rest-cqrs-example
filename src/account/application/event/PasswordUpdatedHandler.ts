...
import { AccountPasswordUpdated, IntegrationEventPublisher, INTEGRATION_EVENT_PUBLISHER, Topic } from "libs/MessageModule";
...
export class PasswordUpdatedHandler implements IEventHandler<PasswordUpdatedEvent> {
  ...
  async handle(event: PasswordUpdatedEvent): Promise<void> {
    await this.integrationEventPublisher.publish(Topic.ACCOUNT_PASSWORD_UPDATED, new AccountPasswordUpdated(event.accountId, event.email));
  }
}
