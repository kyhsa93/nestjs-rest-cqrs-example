...
import { AccountOpened, IntegrationEventPublisher, INTEGRATION_EVENT_PUBLISHER, Topic } from "libs/MessageModule";
...
export class AccountOpenedHandler implements IEventHandler<AccountOpenedEvent> {
  ...
  async handle(event: AccountOpenedEvent): Promise<void> {
    await this.integrationEventPublisher.publish(Topic.ACCOUNT_OPENED, new AccountOpened(event.accountId, event.email))
  }
}
